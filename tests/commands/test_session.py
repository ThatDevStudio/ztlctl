...
class TestSessionCommands:
    def test_session_start(self, cli_runner: CliRunner) -> None:
        result = cli_runner.invoke(cli, ["session", "start", "My Topic"])
        assert result.exit_code == 0
        assert "session_start" in result.output

    def test_session_start_json(self, cli_runner: CliRunner) -> None:
        result = cli_runner.invoke(cli, ["--json", "session", "start", "JSON Topic"])
        assert result.exit_code == 0
        import json
    ...
    def test_session_close(self, cli_runner: CliRunner) -> None:
        # Start first
        cli_runner.invoke(cli, ["session", "start", "Close Topic"])
        result = cli_runner.invoke(cli, ["session", "close"])
        assert result.exit_code == 0

    def test_session_close_no_active(self, cli_runner: CliRunner) -> None:
        result = cli_runner.invoke(cli, ["--json", "session", "close"])
    ...
    def test_session_reopen(self, cli_runner: CliRunner) -> None:
        ...
        start_result = cli_runner.invoke(cli, ["--json", "session", "start", "Reopen Topic"])
        assert start_result.exit_code == 0
        session_id = json.loads(start_result.output)["data"]["id"]

        cli_runner.invoke(cli, ["session", "close"])
        result = cli_runner.invoke(cli, ["session", "reopen", session_id])
        assert result.exit_code == 0

    def test_session_reopen_not_found(self, cli_runner: CliRunner) -> None:
        result = cli_runner.invoke(cli, ["session", "reopen", "LOG-9999"])
        assert result.exit_code == 1

    def test_session_reopen_already_open_json_writes_to_stderr_only(
        self, cli_runner: CliRunner
    ) -> None:
        start_result = cli_runner.invoke(cli, ["--json", "session", "start", "Already Open Topic"])
        session_id = json.loads(start_result.output)["data"]["id"]

        result = cli_runner.invoke(cli, ["--json", "session", "reopen", session_id])
